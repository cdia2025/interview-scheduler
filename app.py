...
# ================= CONFIGURATION =================
st.set_page_config(page_title="Interview Scheduler", layout="wide", page_icon="Hz")
...
def safe_add_record(new_row_df):
    ...
    try:
        with st.spinner("🔒 安全寫入中 (正在同步雲端最新資料)..."):
            ...
            st.session_state.data_revision += 1 # 強制月曆更新 Key
...
with tab1:
    if not df.empty:
        # 重設 Index 確保每個事件有獨立 ID
        df_cal = df.reset_index(drop=True)
        events = []
        for index, row in df_cal.iterrows():
            # 嚴格檢查日期時間格式
            if row['Date'] and row['Time'] and len(row['Date']) == 10 and len(row['Time']) == 5:
                try:
                    # 再次確保格式正確 YYYY-MM-DD T HH:MM
        ...
        calendar(events=events, options={
            ...
            "height": "750px",  # 固定高度防止消失
            "headerToolbar": {"left": "prev,next today", "center": "title", "right": "dayGridMonth,listMonth"},
            "eventTimeFormat": {"hour": "2-digit", "minute": "2-digit", "hour12": False},
            "handleWindowResize": True, # 幫助切換分頁時重新調整
...
with tab2:
    ...
    with c2:
        ...
        if st.button("💾 儲存網格變更 (覆蓋模式)", type="secondary"):
            clean_out = out.copy()
            clean_out['Date'] = clean_out['Date'].apply(lambda x: x.strftime('%Y-%m-%d') if x is not None and str(x) != 'NaT' else '')
            clean_out['Time'] = clean_out['Time'].apply(lambda x: x.strftime('%H:%M') if x is not None and str(x) != 'NaT' else '')
